...
ALLEGRO_FONT *game_ttf_12;
ALLEGRO_BITMAP *game_picture;
...
int main(void) {
    ...
    if(!al_init_primitives_addon())
        game_abort("failed to initialize primitives addon");
    if(!al_init_image_addon())
        game_abort("failed to initialize image addon");
    if(!al_init_font_addon())
        game_abort("failed to initialize font addon");
    if(!al_init_ttf_addon())
        game_abort("failed to initialize ttf addon");
...
void game_init(void) {
    ...
    game_ttf_12 = al_load_ttf_font("pirulen.ttf", 12, 0);
    if(!game_ttf_12)
        game_abort("failed to load ttf font");
    
    game_picture = al_load_bitmap("32largebugs.jpg");
    if(!game_ttf_12)
        game_abort("failed to load pitcure");
...
void game_draw(void) {
    ...
    al_draw_rectangle((SCREEN_W-437)/2, 24, (SCREEN_W+437)/2, 50, al_map_rgb(255, 255, 255), 1);
    al_draw_text(game_ttf_12, al_map_rgb(255, 255, 255), SCREEN_W/2, 30, ALLEGRO_ALIGN_CENTER, "How to deal with bugs in your final project");
    al_draw_bitmap(game_picture, (SCREEN_W - IMG_W) / 2, (SCREEN_H - IMG_H) / 2, 0);
    al_draw_text(game_ttf_12, al_map_rgb(255, 255, 255), SCREEN_W / 2, 550, ALLEGRO_ALIGN_CENTER, "Source: http://cartoontester.blogspot.com/2010/01/big-bugs.html");
...
void game_destroy(void) {
    ...
    al_destroy_font(game_ttf_12);
    al_destroy_bitmap(game_picture);
