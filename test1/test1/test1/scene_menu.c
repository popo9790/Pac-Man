...
static void init() {
    ...
    history_score1 = fopen("Assets/history_score.txt", "r");
    if(!history_score1){
        game_abort("failed to open history_score.txt");
    }
    for(int i=0; i<3; ++i){
        fscanf(history_score1, "%d", &high_score[i]);
    }
    fclose(history_score1);
}

static int TitleMove = 0;
static void draw() {
    ...
    al_draw_text(menuFont, al_map_rgb(255, 255, 255),
                 SCREEN_W/2, 40,
                 ALLEGRO_ALIGN_CENTRE, "Highest scores");
    char st_score[200],sec_score[200],thr_score[200];
    sprintf(st_score,"1.st : %03d",high_score[0]);
    sprintf(sec_score,"2.nd : %03d",high_score[1]);
    sprintf(thr_score,"3.rd : %03d",high_score[2]);
    
    al_draw_text(menuFont, al_map_rgb(255, 255, 255),
                  SCREEN_W/2, 80, ALLEGRO_ALIGN_CENTRE, st_score);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255),
                  SCREEN_W/2, 115, ALLEGRO_ALIGN_CENTRE, sec_score);
    al_draw_text(menuFont, al_map_rgb(255, 255, 255),
                  SCREEN_W/2, 150, ALLEGRO_ALIGN_CENTRE, thr_score);
...
static void on_mouse_down() {
	if (btnSettings.hovered && mouse_state[1])
