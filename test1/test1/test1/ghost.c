...
bool ghost_movable(Ghost* ghost, Map* M, Directions targetDirec, bool room) {
    ...
    if(is_wall_block(M, Ghost_x, Ghost_y) || (room && is_room_block(M, Ghost_x, Ghost_y)))
        return false;
...
void ghost_move_script_FLEE(Ghost* ghost, Map* M, const Pacman * const pacman) {
    ...
    for(Directions i=1; i<=4; ++i){
        if(i != shortestDirection && ghost_movable(ghost, M, i, true)){
            prob[cnt++] = i;
        }
    }
    rnd = generateRandomNumber(0, 256) % cnt;
    if(ghost_movable(ghost, M, ghost->objData.preMove, true) && cnt < 2){
        ghost_NextMove(ghost, ghost->objData.preMove);
    }
    else{
        if (cnt >= 2){
            if(ghost->objData.preMove + prob[rnd] != 5){
                ghost_NextMove(ghost, prob[rnd]);
            }
        }
        else if(!ghost_movable(ghost, M, ghost->objData.preMove, true)){
            if(ghost->objData.preMove != prob[rnd]){
                ghost_NextMove(ghost, prob[rnd]);
            }
            else{
